...
function _createRandomReviews(productId) {
  ...
  for (let i = 0; i < _getRandomArbitrary(2,4); i++)
    reviews.push({
      userName: faker.internet.userName(),
      text: faker.random.words(_getRandomArbitrary(2,5)),
      product: productId
    })
...
router.get('/generate-fake-data', (req, res, next) => {
  for (let i = 0; i < 90; i++) {
    let product = new Product({
      ...
      image: `https://picsum.photos/200/300/?image=${_getRandomArbitrary(1, 200)}`
    })
